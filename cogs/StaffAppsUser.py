...
class embeds():
    ...
    def min_reqs(self, msg_count: int, join_time: datetime, created_at: datetime) -> discord.Embed:
        """
        User does not meet the minimum requirements to apply for staff.
        """
        time_since_creation = (
            datetime.now(tz=created_at.tzinfo) - created_at
        )
...
class user():
    def __init__(self, user: discord.Member) -> None:
        ...
        self.messages_sent = cursor.execute('SELECT messagesSent FROM user WHERE uid = ?' , (self.uid,)).fetchone()[0]
        self.marked_spam = cursor.execute('SELECT markedSpam FROM user WHERE uid = ?' , (self.uid,)).fetchone()[0]
        self.cooldown = float(cursor.execute('SELECT cooldown FROM user WHERE uid = ?' , (self.uid,)).fetchone()[0] or 0)
        # get all statuses
        self.status = [x[0] for x in cursor.execute('SELECT status FROM application WHERE uid = ?' , (self.uid,)).fetchall()]
    ...
    def min_reqs(self) -> tuple[bool, int, datetime]:
        ...
        if self.joined_at <= (datetime.now(tz=self.joined_at.tzinfo) - timedelta(days=30)) and self.messages_sent >= 500:
...
class StaffAppView(discord.ui.View):
    ...
    async def button_callback(self, button, interaction):
        ...
        await interaction.response.send_modal(StaffAppModal(self.author, self.nda, self.timezone, self.time_per_wk, self.bot, title="Input answers"))


class StaffAppModal(discord.ui.Modal):
    def __init__(self, author, nda, timezone ,hours_per_wk, bot, *args, **kwargs) -> None:
        """
...
class StaffAppsUser(commands.Cog):
    ...
    async def apply(self, ctx: commands.Context) -> None:
        ...
        if applicant.is_on_cooldown():
            await ctx.respond(embed=embeds().cooldown(applicant.cooldown), ephemeral=True)
            Log(f"{logname} tried to apply for staff but is on cooldown")
            return

        # check app status
        status = any([x in [1,3,4,5,6,7,8] for x in applicant.status]) #2 and 8 are closed applications, others are open
        if status:
            await ctx.respond(embed=embeds().ongoing(), ephemeral=True)
            Log(f"{logname} tried to apply for staff but already has an ongoing application")
            return

        # check minimum requirements
        if not applicant.min_reqs()[0]:
            await ctx.respond(embed=embeds().min_reqs(applicant.min_reqs()[1], applicant.joined_at, applicant.created_at), ephemeral=True)
            Log(f"{logname} tried to apply for staff but does not meet the minimum requirements")
            return
        ...
        **Non-disclosure agreement**
        ...
        await ctx.respond(embed=embed, ephemeral=True, view=StaffAppView(ctx.author, self.bot))
